                ...
                <ul className="flex justify-between items-center w-96">
                    {
                        navLinkData.map((item, index)=>(
                            <a href={ item.link } key={ index }>
                                <li className="font-montserrat text-text_secondary text-sm font-medium">{ item.title }</li>
                            </a>
                        ))
                    }
                </ul>

                <div className="absolute w-full h-[1px] bg-accent bottom-0 bg-blend-difference"></div>
            </div>
        </nav>
    </>
  )
